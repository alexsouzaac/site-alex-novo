...
const Button = ({ children, variant = 'primary', href, onClick, className = '', ...props }: any) => {
  const baseStyles = "inline-flex items-center justify-center px-6 md:px-8 py-3 text-sm md:text-base font-bold transition-all duration-200 uppercase tracking-wide rounded-sm cursor-pointer relative overflow-hidden group w-full md:w-auto";
  const variants = {
...
const PartnerTicker = ({ onNavigate }: { onNavigate: () => void }) => {
  ...
  return (
    <div 
      className="w-full bg-neutral-950 border-b border-neutral-800 py-3 md:py-4 overflow-hidden relative cursor-pointer group select-none z-30 flex flex-col items-center gap-3"
      onClick={onNavigate}
    >
      {/* Título Centralizado no Carrossel */}
      <span className="text-[10px] md:text-xs font-bold text-accent-500 uppercase tracking-[0.2em] relative z-20 bg-neutral-950/50 px-3 py-0.5 rounded-full border border-neutral-800">
        Nossos Parceiros
      </span>

      <div className="w-full relative">
        <div className="absolute left-0 top-0 bottom-0 w-8 md:w-32 z-10 bg-gradient-to-r from-neutral-950 to-transparent pointer-events-none"></div>
        <div className="absolute right-0 top-0 bottom-0 w-8 md:w-32 z-10 bg-gradient-to-l from-neutral-950 to-transparent pointer-events-none"></div>

        <motion.div 
          className="flex items-center gap-8 md:gap-16 w-max pl-4"
          animate={{ x: ["0%", "-50%"] }}
          transition={{ repeat: Infinity, ease: "linear", duration: 30 }}
        >
          {tickerItems.map((partner, idx) => (
             <div key={idx} className="flex items-center gap-3 md:gap-4 opacity-90 md:opacity-60 group-hover:opacity-100 transition-opacity duration-300">
                <div className="w-10 h-10 md:w-8 md:h-8 rounded-full overflow-hidden border border-neutral-800 bg-neutral-900 shrink-0">
                  <img src={partner.image} alt={partner.name} className="w-full h-full object-cover md:grayscale md:group-hover:grayscale-0 transition-all" />
                </div>
                <div>
      ...
      >
        <div className="aspect-video w-full bg-black relative overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/40 transition-colors">
            <motion.div className="w-12 h-12 md:w-14 md:h-14 bg-red-600 rounded-full flex items-center justify-center shadow-2xl backdrop-blur-sm" whileHover={{ scale: 1.2 }}>
              <PlayCircle className="w-5 h-5 md:w-6 md:h-6 text-white fill-white ml-1" />
            </motion.div>
          </div>
        </div>
        <div className="p-4 flex items-center justify-between bg-neutral-900 relative z-10">
          <div className="pr-2">
            <h3 className="font-bold text-white text-sm md:text-base leading-tight group-hover:text-red-500 transition-colors line-clamp-1">{video.title}</h3>
            <p className="text-neutral-500 text-[10px] uppercase tracking-widest font-bold mt-1">{video.category}</p>
          </div>
          <Youtube className="w-5 h-5 text-neutral-600 group-hover:text-red-500 transition-colors shrink-0" />
        </div>
      </motion.a>
    </FadeIn>
  );
    ...
    <FadeIn delay={idx * 0.1}>
      ...
      >
        <div className="aspect-square w-full bg-gradient-to-br from-purple-600 via-pink-600 to-orange-500 relative flex items-center justify-center overflow-hidden">
          ...
          <Instagram className="w-12 h-12 md:w-16 md:h-16 text-white drop-shadow-lg transform group-hover:scale-110 group-hover:rotate-6 transition-all duration-300" />
        ...
        <div className="p-4 bg-neutral-900 relative z-10 flex-1 flex flex-col justify-center">
          <h3 className="font-bold text-white text-sm md:text-base leading-tight group-hover:text-pink-500 transition-colors">{post.title}</h3>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans selection:bg-accent-500 selection:text-white overflow-x-hidden flex flex-col">
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="absolute top-16 left-0 w-full bg-neutral-950 border-b border-neutral-800 p-4 flex flex-col gap-2 md:hidden shadow-2xl h-[calc(100vh-64px)] overflow-y-auto"
            >
              {navItems.map((item) => (
                <button 
                  key={item.id} 
                  onClick={() => navigateTo(item.id)}
                  className={`text-lg font-medium py-4 px-4 rounded-sm text-left transition-colors flex items-center gap-4 ${
              ...
              <div className="mt-auto mb-8">
                 <Button href={socialLinks.whatsapp} className="w-full">Fazer Orçamento</Button>
                 <div className="flex justify-center mt-6 gap-6">
                    <a href={socialLinks.instagram} target="_blank" className="text-neutral-400 hover:text-white"><Instagram className="w-8 h-8" /></a>
                    <a href={socialLinks.youtube} target="_blank" className="text-neutral-400 hover:text-white"><Youtube className="w-8 h-8" /></a>
                 </div>
              </div>
            </motion.div>
          )}
        ...
        <AnimatePresence mode="wait">
          ...
          {currentPage === 'inicio' && (
            <motion.div key="inicio" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col">
              ...
              {/* TICKER DE PARCEIROS */}
              <div className="w-full z-20 border-b border-neutral-800/50">
                 <PartnerTicker onNavigate={() => navigateTo('parcerias')} />
              </div>

              <section className="relative flex-grow py-12 md:py-20 flex flex-col items-center justify-center overflow-hidden min-h-[60vh]">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] md:w-[500px] h-[300px] md:h-[500px] bg-accent-600/20 rounded-full blur-[80px] md:blur-[120px] pointer-events-none animate-pulse-slow"></div>
                ...
                <div className="max-w-4xl mx-auto px-4 text-center relative z-10 mt-4 md:mt-8 w-full">
                  <FadeIn>
                    <motion.div className="inline-flex items-center gap-2 mb-4 md:mb-6 px-3 py-1.5 bg-neutral-900/80 backdrop-blur-md border border-accent-500/30 rounded-full text-[10px] md:text-xs font-bold tracking-widest text-accent-400 uppercase shadow-lg shadow-accent-500/10 cursor-default">
                      <Sparkles className="w-3 h-3" />
                      Especialista em produção de vídeos com IA
                    </motion.div>
                    <h1 className="text-3xl md:text-6xl lg:text-7xl font-extrabold text-white tracking-tight leading-tight mb-4 md:mb-6">
                    ...
                    <p className="text-base md:text-xl text-neutral-400 mb-8 max-w-xl md:max-w-2xl mx-auto leading-relaxed px-2">
              ...
              <section className="py-8 md:py-12 border-t border-neutral-900 bg-neutral-950 mt-auto">
                 <div className="max-w-6xl mx-auto px-4 flex flex-wrap justify-center gap-4 md:gap-8 text-neutral-500 font-medium text-xs md:text-sm">
                    <span className="flex items-center gap-2 bg-neutral-900/50 px-3 py-1.5 rounded-full border border-neutral-800"><Cpu className="w-4 h-4 text-accent-500"/> Inteligência Artificial</span>
                    <span className="flex items-center gap-2 bg-neutral-900/50 px-3 py-1.5 rounded-full border border-neutral-800"><CheckCircle2 className="w-4 h-4 text-accent-500"/> Alta Definição</span>
                    <span className="flex items-center gap-2 bg-neutral-900/50 px-3 py-1.5 rounded-full border border-neutral-800"><Clock className="w-4 h-4 text-accent-500"/> Entrega Rápida</span>
                 </div>
              </section>
            </motion.div>
          )}
              ...
              <section className="py-12 md:py-20 px-4">
                <div className="max-w-4xl mx-auto">
                   <FadeIn className="text-center mb-10 md:mb-16">
                      <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Sobre o Profissional</h2>
                      <div className="w-20 h-1 bg-accent-500 mx-auto rounded-full"></div>
                   </FadeIn>
                   
                   <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center">
                      <div className="bg-neutral-900 p-6 md:p-8 border border-neutral-800 rounded-sm relative">
                        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-accent-500 to-purple-600"></div>
                        <h3 className="text-xl md:text-2xl font-bold text-white mb-4">Alex Souza</h3>
                        <p className="text-neutral-400 leading-relaxed mb-6 font-medium text-base md:text-lg">
                        ...
                        <ul className="space-y-3 text-neutral-300 text-sm md:text-base">
                          <li className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-accent-500 shrink-0 mt-0.5" /> Criação de Histórias (Infantis, Comunicados, Promoção)</li>
                          <li className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-accent-500 shrink-0 mt-0.5" /> Consultoria para elaboração de conteúdo para suas redes Sociais</li>
                          <li className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-accent-500 shrink-0 mt-0.5" /> Edição avançada com Inteligência Artificial</li>
                        </ul>
                      </div>
                    ...
                    <div className="grid md:grid-cols-3 gap-6 md:gap-8 mb-20">
                      {services.map((service, idx) => (
                        <FadeIn key={idx} delay={idx * 0.1}>
                          <div className="bg-neutral-900/60 backdrop-blur-sm p-6 md:p-8 h-full border border-neutral-800 hover:border-accent-500 hover:bg-neutral-900 transition-all duration-300 rounded-lg group relative overflow-hidden shadow-lg">
                            {/* Glow Effect */}
                            ...
                            <service.icon className="w-10 h-10 md:w-12 md:h-12 text-white bg-accent-600 p-2.5 rounded-lg mb-6 shadow-lg shadow-accent-500/20 group-hover:scale-105 transition-transform" />
                            
                            <h3 className="text-lg md:text-xl font-bold text-white mb-1">{service.title}</h3>
                            <p className="text-[10px] md:text-xs font-bold text-accent-500 uppercase tracking-widest mb-6">{service.subtitle}</p>
              ...
              <section className="py-12 md:py-20 px-4">
                <div className="max-w-6xl mx-auto">
                   <FadeIn className="text-center mb-10 md:mb-16">
                      ...
                      <p className="text-neutral-400 text-sm md:text-base">Trabalhos recentes selecionados.</p>
                   </FadeIn>

                   <div className="mb-12 md:mb-20">
                      <div className="flex items-center gap-4 mb-6 md:mb-8 border-b border-neutral-800 pb-4">
                        <Youtube className="text-red-500 w-6 h-6" />
                        <h3 className="text-lg md:text-xl font-bold text-white">Destaques YouTube</h3>
                      </div>
                   ...
                   <div>
                      <div className="flex items-center gap-4 mb-6 md:mb-8 border-b border-neutral-800 pb-4">
                        ...
                        <h3 className="text-lg md:text-xl font-bold text-white">Destaques Instagram</h3>
                      </div>
                      <div className="grid md:grid-cols-3 gap-6">
          ...
          {currentPage === 'parcerias' && (
            <motion.div key="parcerias" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen">
              <section className="py-12 md:py-20 px-4">
                <div className="max-w-6xl mx-auto">
                   <FadeIn className="text-center mb-10 md:mb-16">
                      <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Nossas Parcerias</h2>
                      <p className="text-neutral-400 max-w-2xl mx-auto text-sm md:text-base">
          ...
          {currentPage === 'contato' && (
             <motion.div key="contato" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="min-h-screen flex flex-col justify-center">
                <section className="py-12 md:py-20 px-4">
                   <div className="max-w-4xl mx-auto">
                      <div className="text-center bg-gradient-to-b from-neutral-900 to-neutral-950 border border-neutral-800 p-6 md:p-20 rounded-lg relative overflow-hidden shadow-2xl">
                          ...
                          <div className="flex justify-center mb-6 md:mb-8 relative z-10">
                            <div className="w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-neutral-800 shadow-2xl ring-2 ring-accent-500/50">
                              <img src={profileImage} alt="Alex Souza" className="w-full h-full object-cover" />
                            </div>
                          </div>

                          <h2 className="text-2xl md:text-5xl font-bold text-white mb-4 md:mb-6 leading-tight relative z-10">
                            Vamos tirar seu projeto do papel?
                          </h2>
                          <p className="text-base md:text-lg text-neutral-400 mb-8 md:mb-10 max-w-2xl mx-auto leading-relaxed relative z-10">
                            Entre em contato agora para um orçamento personalizado. Sem compromisso, direto e rápido.
                          </p>
                          
                          <div className="relative z-10 flex flex-col sm:flex-row justify-center gap-4 mb-12 md:mb-16">
                            <Button href={socialLinks.whatsapp} className="py-4 md:py-5 px-6 md:px-8 text-base md:text-lg w-full sm:w-auto shadow-xl shadow-accent-600/20">
                              <span className="flex items-center justify-center gap-3">
                                <MessageCircle className="w-5 h-5 md:w-6 md:h-6" />
                                Chamar no WhatsApp
                              </span>
                            </Button>
                            <Button href={socialLinks.instagram} variant="secondary" className="py-4 md:py-5 px-6 md:px-8 text-base md:text-lg w-full sm:w-auto">
                              <span className="flex items-center justify-center gap-3">
                          ...
                          <div className="text-left border-t border-neutral-800 pt-8 md:pt-10">
                            <h3 className="text-lg md:text-xl font-bold text-white mb-6 text-center">Dúvidas Frequentes</h3>
                              ...
                              {faqs.map((faq, idx) => (
                                <div key={idx} className="border border-neutral-800 bg-neutral-900/50 rounded-sm overflow-hidden hover:border-neutral-700 transition-colors">
                                  <button onClick={() => toggleFaq(idx)} className="w-full flex items-center justify-between p-4 text-left">
                                    ...
                                    {openFaq === idx ? <ChevronUp className="w-4 h-4 text-accent-500 shrink-0 ml-2" /> : <ChevronDown className="w-4 h-4 text-neutral-500 shrink-0 ml-2" />}
                                  </button>
      ...
      {/* FOOTER FIXO */}
      <footer className="bg-neutral-950 border-t border-neutral-900 py-6 md:py-8 text-center text-xs md:text-sm text-neutral-600">
